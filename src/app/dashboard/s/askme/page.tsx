...
const modelConfig = {
  "gemini-2.5-flash-lite": { label: "Lite", icon: Zap, description: "Fast & light" },
  "gemini-2.5-flash": { label: "Fast", icon: Cpu, description: "Balanced" },
  "gemini-3-flash-preview": { label: "Pro", icon: Brain, description: "Most capable" },
} as const;
...
export default function ChatInterface() {
  ...
  const CurrentModelIcon = modelConfig[model].icon;

  return (
    <div className="flex flex-col h-[calc(100vh-4rem)] w-full max-w-3xl mx-auto relative overflow-hidden">
      ...
      {!hasMessages && !isTyping && (
        <div className="flex flex-col items-center justify-center flex-1 px-4 py-8 space-y-4 animate-in fade-in duration-500">
          ...
          <p className="text-sm sm:text-base text-muted-foreground text-center max-w-md px-4">
            I'm <span className="font-semibold text-blue-500">Saify</span>. How can I help you today?
          </p>

          {/* Quick Suggestion Chips */}
          <div className="flex flex-wrap justify-center gap-2 mt-4 px-4 max-w-lg">
            {["Check my attendance", "View my fees", "Room information"].map((suggestion) => (
              <button
                key={suggestion}
      ...
      >
        <form
          onSubmit={handleSend}
          className="relative w-full group bg-muted/40 backdrop-blur-xl border border-border/80 rounded-2xl sm:rounded-[24px] p-2 transition-all focus-within:ring-2 focus-within:ring-blue-500/30 focus-within:border-blue-500/50 shadow-lg shadow-black/5"
        >
          <div className="flex items-end gap-2 px-1 sm:px-2">
            ...
            >
              <SelectTrigger className="w-auto h-9 px-2 sm:px-3 text-xs rounded-xl border-none bg-muted/60 hover:bg-muted focus:ring-0 gap-1.5 shrink-0">
                <CurrentModelIcon className="w-3.5 h-3.5 text-blue-500" />
